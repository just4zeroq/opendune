...
class OKXCollector:
    ...
    def __init__(
        ...
        self.secret = secret or settings.okx_secret_key
        self.passphrase = passphrase or settings.okx_passphrase
        self.testnet = testnet or settings.okx_testnet
        ...
        logger.info("okx_collector_initialized", testnet=self.testnet)

    async def start(self):
        """启动采集器"""
        self.running = True
        logger.info("okx_collector_started")
    ...
    async def stop(self):
        ...
        logger.info("okx_collector_stopped")
    ...
    async def _ticker_loop(self, symbol: str):
        ...
        while self.running:
            try:
                ...
                logger.error("okx_ticker_error", symbol=symbol, error=str(e))
    ...
    async def _trades_loop(self, symbol: str):
        ...
        while self.running:
            try:
                ...
                logger.error("okx_trades_error", symbol=symbol, error=str(e))
                await asyncio.sleep(5)
    ...
    async def _klines_loop(self, symbol: str, interval: str = "1m"):
        ...
        while self.running:
            try:
                ...
                logger.error("okx_klines_error", symbol=symbol, error=str(e))
                await asyncio.sleep(5)
